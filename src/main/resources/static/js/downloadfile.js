...
function getStudent() {
   // $("#btn").prop("disabled", true);
    ...
    // ---------------------
    /*$.ajax({
        url: '/api/students/get-all-students',
        type: 'GET',
        success: function (response) {
            let items = response
...
function saveStudentDocument(){
    var nationalID = $('#fileUploadForm')[0];
    var academicEligibilityForm = $('#fileUploadForm')[1];
    var consentFormForm = $('#fileUploadForm')[2];
    var physcicalExaminationForm= $('#fileUploadForm')[3];
    var applicationLetterForm = $('#fileUploadForm')[4];


    var ajaxData = new FormData(nationalID)
    ajaxData.append(academicEligibility,academicEligibilityForm.files[0])
    ajaxData.append(consentForm,consentFormForm.files[0])
    ajaxData.append(physcicalExamination,physcicalExaminationForm.files[0])
    ajaxData.append(applicationLetter,applicationLetterForm.files[0])


    let tenantId = JSON.parse(localStorage.getItem("tenant_id"));
    ...
    $("#btnSubmit").prop("disabled", false);
        $.ajax({
            ...
            url: "/api/student/uploadstudentDocument",
            data: ajaxData,
            processData: false,
            contentType: false,
            cache: false,
